...
| `/brain-sync [--diff]` | Detects architecture drift after `git pull`, inspects git delta, and auto-syncs Ground Truth & sequences. (Alias: `/brain-diff`). |
...
### 4. `/brain-sync [--diff]`: Living Architecture & Documentation Drift Synchronizer
Keep Second Brain deliverables permanently aligned with live codebases after upstream developers push changes or after running `git pull`:
```bash
# Safe, view-only Architecture Drift & Gap Analysis
/brain-sync --diff
# or alias:
/brain-diff
...
npm run sync
```
Inspects upstream git branches and latest commits across connected microservices, detects new DDL tables or modified endpoints, highlights missing sequence diagrams, and auto-generates 1-to-1 PlantUML diagrams with 100% provenance tags.
